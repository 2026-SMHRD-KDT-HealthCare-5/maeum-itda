...
`senior-daily-record`(UC-14/FR-01-09)와 `guardian-weekly-report`(UC-15/FR-03-07, `GUARDIAN_HOME_01`과는 별도 화면으로 요구사항정의서에서 확정됨)는 화면설계서에는 여전히 화면ID가 없지만, Figma에 각각 '시니어 이전 대화 기록 조회 화면'·'보호자 주간 리포트 조회'로 목업이 추가되어(`docs/screens/`) 결정사항 로그 §7에서 실제로 구현됐습니다. `guardian-weekly-report`는 §6/§7에 따라 `guardian-report`와 탭 위젯(`widgets/report-period-tabs`)을 공유하는 방식으로 진입합니다 — 페이지/라우트(`/guardian/report/weekly/:weekStart`)는 그대로 두고, 두 화면이 동일한 탭 pill을 렌더링해 시각적으로 하나의 셸처럼 보이게 했습니다. 페이지 자체를 완전히 병합할지는 여전히 열려 있는 후속 논의 대상입니다. `senior-daily-record`는 `features/select-daily-record-date`(날짜 네비게이션 + 캘린더 모달)와 `entities/conversation`의 `DailyConversationList`, 그리고 하루 요약 카드로 구성되어 있습니다 — 실제 API 연결 전이라 날짜별 turn과 요약 코멘트는 페이지 로컬 mock 데이터입니다.

**`senior-my-info` / `guardian-my-info`는 결정사항 로그 §7에서 처음으로 실제 화면이 됐습니다.** 화면설계서의 메뉴구성에는 이름만 있던 화면이었는데, 이제 Figma 목업('시니어 내 정보'·'보호자 내 정보')을 기준으로 구현했습니다. 두 화면 모두 `features/edit-basic-info`(기본 정보 인라인 편집)와 `features/disconnect-connection`(연결 끊기) 카드를 공유하고, `senior-my-info`는 여기에 `features/set-checkin-reminder`(안부 알림 시간), `guardian-my-info`는 `features/set-notification-threshold`(UC-12, 정서 지수 알림 임계치)를 추가로 갖습니다. **`guardian-notification-settings` 페이지와 그 라우트(`/guardian/notification-settings`)는 이 흡수에 따라 삭제됐습니다** — UC-12 관련 UI는 이제 `guardian-my-info`에만 있습니다. 실제 API 연결 전이라 시니어/보호자/연결 데이터는 각 페이지의 로컬 mock 상태입니다.
...
- `entities/*`는 `model/`에 실제 TypeScript 타입이 있습니다(화면 설명 + 결정사항 로그에 근거하며, 미해결 사항은 주석으로 표시 — 예를 들어 `entities/notification`의 `target` 구조는 결정사항 로그 §2 기준으로 아직 미해결입니다), `ui/api/lib`는 대부분 placeholder 껍데기이지만 예외가 늘었습니다: `entities/report/ui`는 결정사항 로그 §7에서 `EmotionScoreCard`/`RecommendedActionCard`/`ConversationSummaryCard`/`WeeklyStatsCards`/`WeeklyDailySummaryList`로 실제 구현됐고(`guardian-home`/`guardian-report`/`guardian-weekly-report`가 사용), `entities/conversation/ui`도 실시간 대화용 `ConversationHistoryList`에 더해 특정 날짜 turn을 보여주는 `DailyConversationList`가 추가됐습니다(`senior-daily-record`가 사용). `entities/connection/lib`에는 `daysSinceConnected`/`formatConnectionDuration` 헬퍼가 실제로 있습니다. `entities/conversation`의 `ConversationTurn.sentimentLabel`/`sentimentNote`와 `entities/report`의 `DailyReport.conversationSummary`/`recommendedAction`은 요구사항정의서 최종본(UC-04/UC-06-4/UC-07)에 맞춰 추가된 필드입니다. `entities/report`의 `EvidenceSentence.sentimentLabel`·`WeeklyReport.maxScore`/`minScore`/`dailyScores[].comment`, `entities/senior`의 `username`/`phone`/`checkinReminder`, `entities/connection`의 `ConnectionStatus`에 추가된 `'disconnected'`와 `Connection.connectedAt`은 결정사항 로그 §7의 Figma 화면 감사에서 추가된 필드입니다. `voice_score`/`VoiceScore`(음성 점수 수치화)는 한때 검토됐다가 결정사항 로그 §2-5에 따라 완전히 폐기됐으니 새로 추가하지 마세요 — `emotionScore`는 TextScore 단일값입니다. `entities/connection`(UC-00-1 연결 상태)은 시니어·보호자 어느 한쪽에도 속하지 않는 둘 사이의 관계라서 별도로 뺐습니다 — `entities/senior`의 `connectedGuardianId`, `entities/guardian`의 `connectedSeniorId`는 각 역할 쪽에서 보는 최소 참조 필드일 뿐, 연결 요청 자체의 상태는 여기서 다룹니다.
