...
- `shared/api`에 실제 `ChatSocket` 클래스(`wsClient.ts`)가 있습니다 — `/ws/chats`의 envelope(`event`/`payload`/`ts`) 프로토콜을 구현하고, `auth`/`chat:start`/`chat:end`/`audio:metadata`+바이너리 전송과 `auth:success`/`auth:error`/`ai:question`/`audio:ack`/`chat:idle-warning`/`chat:ended`/`chat:restored`/`error` 수신을 타입 이벤트 리스너(`on`/`off`)로 제공합니다. `docs/ws-protocol.md`가 계약 기준입니다. AI 서버는 이미 TTS 오디오를 만들어내지만, 백엔드가 아직 `tts:audio` 류 WS 이벤트로 중계하지 않아 `ChatSocket`도 TTS 바이너리 수신을 다루지 않습니다(자세한 내용은 docs/ws-protocol.md §8 참고) — 프론트의 TTS 재생 기능(`features`쪽) 자체는 이미 구현되어 이 이벤트가 추가되는 즉시 연결할 수 있는 상태입니다.
