...
| `senior-daily-record` | — (화면설계서 본문에 화면ID 미배정, Figma '시니어 이전 대화 기록 조회 화면' 참고 — UC-14/FR-01-09, 아래 참고) | UC-14 |
...
`senior-daily-record`(UC-14/FR-01-09)와 `guardian-weekly-report`(UC-15/FR-03-07, `GUARDIAN_HOME_01`과는 별도 화면으로 요구사항정의서에서 확정됨)는 화면설계서에는 여전히 화면ID가 없지만, Figma에 각각 '시니어 이전 대화 기록 조회 화면'·'보호자 주간 리포트 조회'로 목업이 추가됐고 `docs/screens/`에 PNG가 들어와 있습니다. `guardian-weekly-report`는 결정사항 로그 §6에 따라 진입 방식이 바뀌었습니다 — `GUARDIAN_HOME_01`의 '상세 보기'가 아니라 `guardian-report` 화면 상단의 일간/주간 탭에서 진입합니다. 별도 페이지/라우트(`/guardian/report/weekly/:weekStart`)를 그대로 두고 탭에서 이동만 시킬지, `guardian-report`에 탭 상태로 흡수할지는 아직 미정이니 구현 시 확정하세요. 둘 다 지금은 다른 placeholder와 동일 수준으로 스캐폴딩만 돼 있고(각각 시니어 홈 화면/`guardian-report`의 임시 링크로 진입) — Figma 디자인이 실제로 반영되는 대로 이 placeholder들을 교체하고 이 표도 함께 갱신하세요.

**`senior-my-info` / `guardian-my-info`는 그동안 실제 스펙이 없었습니다.** 화면설계서의 메뉴구성에는 시니어 화면과 보호자 화면 양쪽 모두에 "내 정보"라는 항목이 있었지만, 그 문서의 다른 모든 화면과 달리 화면 ID도, UC도, 목업 페이지도 없었습니다 — 메뉴 트리 안의 이름 하나뿐이었습니다. 이제 Figma에 각각 '시니어 내 정보'·'보호자 내 정보' 목업이 처음으로 추가되어 `docs/screens/`에 PNG가 들어와 있습니다. 특히 `guardian-my-info`는 기존에 별도 화면이던 `guardian-notification-settings`(UC-12, 알림 임계치 설정)를 흡수할 예정입니다 — `pages/guardian-notification-settings`와 그 라우트를 그대로 두고 `내 정보`에서 진입만 시킬지, `guardian-my-info` 페이지 자체에 `SetNotificationThresholdAction`을 합칠지는 구현 시 확정하세요. 지금은 두 페이지 모두 제목 + 로그인된 `userId` + 실제로 동작하는 로그아웃 버튼(`entities/user`의 `logout()`에 연결)만 있고, 각 역할의 다른 화면들과 동일한 `BottomTabBar`로 감싸져 있습니다 — Figma 디자인이 실제로 반영되는 대로 이 placeholder들을 제대로 다시 만들고 이 표도 함께 갱신하세요.
...
- 라우팅(`app/routes`)은 실제로 동작합니다: `/login`, `/join`, `/senior`, `/senior/conversation`, `/senior/connection`, `/senior/daily-record`, `/senior/my-info`, `/guardian`, `/guardian/report`, `/guardian/report/weekly/:weekStart`, `/guardian/notifications`, `/guardian/notification-settings`, `/guardian/connection`, `/guardian/my-info` 각각이 `entities/user`의 세션/역할을 확인하는 `ProtectedRoute`로 보호되며(`/login`, `/join`은 로그인 전 화면이라 예외), 알 수 없는 경로는 `/login`으로 리다이렉트됩니다. `/`는 `SplashPage`이며 아직 자동 리다이렉트가 없습니다(위 표 참고).
