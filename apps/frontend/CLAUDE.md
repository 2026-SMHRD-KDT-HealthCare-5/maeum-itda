...
- `widgets/*` 4개(`bottom-tab-bar`, `report-period-tabs`, `emotion-trend-chart`, `conversation-timeline`) 모두 실제로 동작합니다 — `bottom-tab-bar`는 page가 넘겨주는 tab item을 그대로 렌더링하며 각 역할의 `SENIOR_TAB_ITEMS`/`GUARDIAN_TAB_ITEMS` 상수도 함께 export합니다(page 안에서 tab 목록을 직접 만들지 말 것), `report-period-tabs`는 `guardian-report`/`guardian-weekly-report`가 공유하는 일간/주간 탭 pill, `emotion-trend-chart`는 Recharts 선그래프(`dailyScores`는 필수 prop이며 항상 `fetchGuardianDashboard`/`fetchWeeklyReport` 등 실데이터를 받습니다 — 데모용 mock 기본값은 없음), `conversation-timeline`은 위험 강조가 있는 근거 문장 접기/펼치기 목록입니다.
