...
- `entities/*`는 `model/`에 실제 TypeScript 타입이 있고(미해결 사항은 주석으로 표시 — 예: `entities/notification`의 `target` 구조는 §2 기준 아직 미해결), `ui/api/lib`는 대부분 placeholder지만 예외가 있습니다: `entities/report/ui`(`EmotionScoreCard`/`RecommendedActionCard`/`ConversationSummaryCard`/`WeeklyStatsCards`/`WeeklyDailySummaryList`), `entities/connection/lib`의 `daysSinceConnected`/`formatConnectionDuration`은 실제 구현됐습니다(결정사항 로그 §7). §7에서 추가된 필드: `EvidenceSentence.sentimentLabel`, `WeeklyReport.maxScore`/`minScore`/`dailyScores[].comment`, `Senior.username`/`phone`/`checkinReminder`, `ConnectionStatus`의 `'disconnected'`와 `Connection.connectedAt`. **`voice_score`/`VoiceScore`는 §2-5에 따라 폐기됐으니 새로 추가하지 마세요** — `emotionScore`는 TextScore 단일값입니다. `entities/connection`(UC-00-1)은 시니어·보호자 어느 쪽에도 속하지 않는 관계라서 별도 엔티티로 분리했습니다. **`entities/conversation`은 apps/backend의 실제 `/ws/chats`·`GET /chats/messages` 계약(docs/ws-protocol.md)에 맞춰 재작성됐습니다** — 예전 `ConversationTurn`(질문/답변 쌍) 모델은 삭제됐고, `packages/shared-types`의 `ChatMessage`(메시지 행 하나: `messageId`/`speakerType`/`content`/`sttStatus`/`createdAt`) 기준입니다. `ConversationHistoryList`/`DailyConversationList` 모두 `messages: ChatMessage[]`를 prop으로 받아 렌더링하는 실제 컴포넌트이고(하드코딩된 mock 대화 없음), `content === null`인 메시지(시니어 답변이 아직 STT 전)는 "답변을 보내드렸어요" placeholder로 표시합니다. `api/index.ts`의 `fetchConversationHistory`도 실제로 `GET /chats/messages`를 호출하지만, 아직 어느 페이지에서도 호출하지 않습니다(시니어 대화 화면은 이번 대화에서 오간 메시지만 보여주고, 과거 이력 복원 연동은 TODO).
- `features/*`는 대부분 네 세그먼트 전부 placeholder 껍데기입니다, **단** 다음은 예외로 실제로 동작합니다: `login-with-credentials`(`model/`의 `mockResolveRole()`이 UC-00의 실제 인증이 반환할 값을 흉내내며, 아이디에 `guardian`이 포함되면 보호자로, 그 외엔 시니어로 로그인 — `apps/backend`가 로그인 엔드포인트를 제공하는 즉시 이 함수 전체를 실제 API 호출로 교체하고 이 문자열 매칭 임시방편을 확장하지 말 것), `register-account`, `view-attendance-calendar`, `start-conversation`, 그리고 결정사항 로그 §7에서 추가·실장된 `edit-basic-info`·`disconnect-connection`·`set-checkin-reminder`·`set-notification-threshold`·`select-daily-record-date`. 나머지(`send-connection-request`/`respond-connection-request`/`mark-notification-read`/`select-report-date`/`view-evidence-sentence`)는 여전히 한 줄짜리 placeholder입니다. **`record-voice-answer`는 이제 실제로 동작합니다** — `model/`의 `useRecordVoiceAnswer` 훅이 AI 질문 도착 시 `getUserMedia`로 마이크 권한을 받아 `MediaRecorder`로 녹음을 시작하고, `lib/`의 `createSilenceWatcher`(Web Audio API 기반 RMS 묵음 감지)로 발화 중 10초 묵음 시 자동 종료하거나 사용자가 "지금 답변 마치기"를 눌러 수동 종료하면(`endType: 'auto' | 'manual'`) `api/`의 `sendVoiceAnswer`를 거쳐 `shared/api`의 `ChatSocket`으로 전송합니다. `audio:ack` 수신 시 시니어 답변을 `ChatMessage`(content는 STT 전이라 null)로 대화 목록에 추가합니다.
- `widgets/*`는 대부분 placeholder 껍데기입니다, `bottom-tab-bar`는 예외로 실제로 동작하며(page가 넘겨주는 tab item을 그대로 렌더링) 각 역할의 모든 page가 넘겨야 할 `SENIOR_TAB_ITEMS`/`GUARDIAN_TAB_ITEMS` 상수도 함께 export합니다 — page 안에서 tab 목록을 직접 만들지 말고 이 상수를 import하세요. 결정사항 로그 §7에서 추가된 `report-period-tabs`도 실제로 동작합니다(`guardian-report`/`guardian-weekly-report`가 공유하는 일간/주간 탭 pill). **`emotion-trend-chart`·`conversation-timeline`도 실제로 동작합니다**(보호자 화면 작업 커밋 `fb06dcb`/`17f7aa4` 등) — 각각 Recharts 선그래프, 위험 강조가 있는 근거 문장 접기/펼치기 목록입니다.
- `shared/api`에 실제 `ChatSocket` 클래스(`wsClient.ts`)가 있습니다 — `/ws/chats`의 envelope(`event`/`payload`/`ts`) 프로토콜을 구현하고, `auth`/`chat:start`/`chat:end`/`audio:metadata`+바이너리 전송과 `auth:success`/`auth:error`/`ai:question`/`audio:ack`/`chat:idle-warning`/`chat:ended`/`chat:restored`/`error` 수신을 타입 이벤트 리스너(`on`/`off`)로 제공합니다. `docs/ws-protocol.md`가 계약 기준이며, TTS 바이너리 수신은 MVP 범위 밖이라 다루지 않습니다.
- 라우팅(`app/routes`)은 실제로 동작합니다: `/login`, `/join`, `/senior`, `/senior/conversation`, `/senior/connection`, `/senior/daily-record`, `/senior/my-info`, `/guardian`, `/guardian/report`, `/guardian/report/weekly/:weekStart`, `/guardian/notifications`, `/guardian/connection`, `/guardian/my-info` 각각이 `entities/user`의 세션/역할을 확인하는 `ProtectedRoute`로 보호되며(`/login`, `/join`은 로그인 전 화면이라 예외), 알 수 없는 경로는 `/login`으로 리다이렉트됩니다. `/`는 `SplashPage`이며 아직 자동 리다이렉트가 없습니다(위 표 참고). `/guardian/notification-settings`는 결정사항 로그 §7에서 `guardian-my-info`로 흡수되며 삭제됐습니다. **`/senior/conversation`(`SeniorConversationPage`)은 이제 실제 `ChatSocket`으로 `/ws/chats`에 연결합니다** — `session.accessToken`이 없으면(현재 로그인은 여전히 mock이라 항상 없음) 연결을 시도하지 않고 "로그인 정보가 없어 대화를 시작할 수 없어요" 안내만 보여줍니다. `apps/backend`에 실제 로그인 엔드포인트가 생겨 `login-with-credentials`가 `Session.accessToken`을 실제 JWT로 채우기 전까지는 이 화면을 브라우저에서 끝까지 확인할 수 없습니다 — 후속 작업 시 이 전제부터 확인하세요.
- `entities/user`의 `Session`에 `accessToken: string | null` 필드가 추가됐습니다(`/ws/chats`의 `auth` 이벤트에 실어 보낼 JWT 자리) — `login-with-credentials`는 실제 로그인 엔드포인트가 없어 항상 `null`로 채웁니다.
