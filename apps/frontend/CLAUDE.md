...
- `shared/api`에 실제 `ChatSocket` 클래스(`wsClient.ts`)가 있습니다 — `/ws/chats`의 envelope(`event`/`payload`/`ts`) 프로토콜을 구현하고, `auth`/`chat:start`/`chat:end`/`audio:metadata`+바이너리 전송과 `auth:success`/`auth:error`/`ai:question`/`audio:ack`/`chat:idle-warning`/`chat:ended`/`chat:restored`/`error` 수신을 타입 이벤트 리스너(`on`/`off`)로 제공합니다. `docs/ws-protocol.md`가 계약 기준입니다. **TTS 재생도 실제로 연결되어 있습니다** — AI 서버가 만든 Typecast 음성을 백엔드가 `tts:audio`(같은 질문의 `ai:question` 직전) WS 이벤트로 중계하고, `pages/senior-conversation`이 이를 messageId로 받아뒀다가 `ai:question` 도착 시 짝지어 `features/record-voice-answer`의 `useRecordVoiceAnswer`에 넘깁니다 — 재생 자체는 `shared/lib`의 `playTtsAudioOnce`가 담당합니다.
