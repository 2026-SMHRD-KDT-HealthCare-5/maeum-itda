...
- **데모 마감: 2026-08-21 — 완료.** 아래 일정은 당시 계획과 완료 이력을 보존한 스냅샷이다.
...
## 현재 상태 (2026-08-25): 데모 완료, 실사용 기반 안정성·온보딩 개선 진행 중

12개 화면 전부 실 API/WS 연동을 완료했고 PWA 설치·웹 푸시도 구현됐다. 2026-08-22~25에는 실기기 하드닝, `gpt-live-transcribe` 기반 부분 전사, 공통 오류/재시도 UI, 스플래시와 계정별 알림 권한 온보딩을 추가했다. **아래 "8/21까지 남은 작업"은 데모 시점 스냅샷이며 현재 판단에는 이 절과 "완료됨"을 우선한다.**
...
- **실시간 STT 전환(2026-08-24)**: 기본 STT를 OpenAI `gpt-live-transcribe` Realtime transcription으로 전환했다. 프론트의 `audio:pcm`을 NestJS와 FastAPI가 중계해 `audio:partial` 자막을 제공하고, 배치 분석도 녹음 파일을 PCM16 24kHz로 디코딩해 같은 모델을 사용한다. 로컬 faster-whisper는 키 무효·쿼터 소진에만 제한적으로 폴백한다.
- **앱 진입 경험 개선(2026-08-25)**: 세션 복원형 스플래시와 계정별 권한 온보딩을 추가했다. 온보딩은 알림 브라우저 권한만 요청하며, 마이크는 브라우저의 사용자 제스처 제한 때문에 실제 안부 대화 시작 시점에만 요청한다.
