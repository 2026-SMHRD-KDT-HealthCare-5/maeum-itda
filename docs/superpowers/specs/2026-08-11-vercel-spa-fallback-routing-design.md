...
`rewrites` 배열은 위에서부터 순서대로 평가된다 — 향후 `/api/*` 프록시 등 추가 rewrite 규칙을 넣을 때는 반드시 이 캐치올(`/(.*)`)  규칙보다 위에 배치해야 한다. 그렇지 않으면 캐치올이 먼저 매칭되어 뒤의 규칙에 절대 도달하지 못한다.
...
- 이 rewrite 적용 이후로는 존재하지 않는 정적 자산(오타난 경로 등)도 404 대신 200 + index.html을 반환한다 — SPA fallback의 일반적인 트레이드오프.
