...
- **apps/ai-server** — Python, FastAPI. STT는 Whisper(faster-whisper) 로컬 모델(GPU 없으면 CPU로 동작)을 사용하고, 꼬리질문 생성·실시간 감성분석(같은 LLM 호출의 Structured Output)은 OpenAI API, TTS는 Typecast(Typecast Streaming API)로 벤더가 확정되어 있습니다. SGDS-K·GAD-7·LSNS-6 3개 척도 매칭 결과만으로 정서지수(TextScore)를 산출하며, 음성 톤·피치 분석 결과는 별도 점수화하지 않고 실시간 감성분석의 입력 재료로만 사용합니다(자세한 내용은 [docs/마음잇다_결정사항_및_이슈로그.md](docs/마음잇다_결정사항_및_이슈로그.md) 참고). STT·척도 채점·꼬리질문 생성·TTS까지 한 턴 파이프라인이 실제 API 호출로 동작하며(`.env`의 모드 플래그를 `model`로 설정해야 함, 위 Repository state 참고), 일간 대화 요약(UC-06-4, `POST /reports/daily-summary`)도 구현되어 있습니다. KLUE 텍스트와 Kresnik 음성 5감정 모델을 `EMOTION_MODE=model`로 실추론하고 온도 보정 후 클래스별 가중합하는 로직도 연결되어 있습니다. 현재 융합 파라미터는 잠가둔 최종 테스트 세트 평가 전 후보값이고, 음성 기반 모델/전처리기는 Hugging Face 캐시 또는 네트워크에 의존합니다. 아직 pnpm 워크스페이스 멤버가 아님 — 위 내용 참고.
