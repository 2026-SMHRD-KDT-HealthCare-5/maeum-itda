...
- **apps/ai-server** — Python, FastAPI. STT는 Whisper(faster-whisper) 로컬 모델(GPU 없으면 CPU로 동작)을 사용하고, 꼬리질문 생성·실시간 감성분석(같은 LLM 호출의 Structured Output)은 OpenAI API, TTS는 Typecast(Typecast Streaming API)로 벤더가 확정되어 있습니다. SGDS-K·GAD-7·LSNS-6 3개 척도 매칭 결과만으로 정서지수(TextScore)를 산출하며, 음성 톤·피치 분석 결과는 별도 점수화하지 않고 실시간 감성분석의 입력 재료로만 사용합니다(자세한 내용은 [docs/마음잇다_결정사항_및_이슈로그.md](docs/마음잇다_결정사항_및_이슈로그.md) 참고). STT·척도 채점·꼬리질문 생성·TTS까지 한 턴 파이프라인이 실제 API 호출로 동작하며(`.env`의 모드 플래그를 `model`로 설정해야 함, 위 Repository state 참고), 일간 대화 요약(UC-06-4, `POST /reports/daily-summary`)도 구현되어 있습니다. 감정 분류 모델(`EMOTION_MODE=model`용 체크포인트)은 아직 없어 그 부분만 목업입니다. 아직 pnpm 워크스페이스 멤버가 아님 — 위 내용 참고.
