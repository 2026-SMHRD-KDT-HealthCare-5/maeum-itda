...
# Runs formatting and workspace lint checks and blocks the commit if either fails.
#
...
if [ "$format_status" -ne 0 ]; then
  truncated="${format_output:0:4000}"
  node -e '
const reason = "pnpm format:check failed — run `pnpm format`, review the changes, then retry the commit (output truncated to 4000 chars):\n\n" + process.argv[1];
